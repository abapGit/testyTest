CLASS zcl_abapgit_xml DEFINITION
  ...
  PROTECTED SECTION.
    DATA: mi_ixml     TYPE REF TO if_ixml_core,
  ...
  PRIVATE SECTION.
    ...
    METHODS error
      IMPORTING
        !ii_parser TYPE REF TO if_ixml_parser_core
      RAISING
    ...
    METHODS raise_exception_for
      IMPORTING
        !ii_error TYPE REF TO if_ixml_parse_error_core
      RAISING
        zcx_abapgit_exception .
ENDCLASS.
...
CLASS zcl_abapgit_xml IMPLEMENTATION.
  ...
  METHOD parse.
    ...
    DATA: li_stream_factory TYPE REF TO if_ixml_stream_factory_core,
          li_istream        TYPE REF TO if_ixml_istream_core,
          li_element        TYPE REF TO if_ixml_element,
          li_version        TYPE REF TO if_ixml_node,
          li_parser         TYPE REF TO if_ixml_parser_core.


    ASSERT NOT iv_xml IS INITIAL.

    li_stream_factory = mi_ixml->create_stream_factory( ).
    li_istream = li_stream_factory->create_istream_xstring( zcl_abapgit_convert=>string_to_xstring_utf8( iv_xml ) ).
...
* will render to codepage UTF-16
    ...
    DATA: li_ostream       TYPE REF TO if_ixml_ostream_core,
          li_renderer      TYPE REF TO if_ixml_renderer_core,
          li_streamfactory TYPE REF TO if_ixml_stream_factory_core.

    li_streamfactory = mi_ixml->create_stream_factory( ).

    DATA foo TYPE xstring. li_ostream = li_streamfactory->create_ostream_xstring( foo ). rv_xml = zcl_abapgit_convert=>xstring_to_string_utf8( foo ).
